...
export function useBusinessExpenses() {
  ...
  const fetchData = async () => {
    try {
      ...
      const gastosResponse = await apiClient<ApiResponse<GastosNegocio[]>>("/business-expenses");
      ...
      // Fetch IVA
      const ivaResponse = await apiClient<ApiResponse<ImpuestoGeneral[]>>("/general-taxes");
  ...
  const deleteGasto = async (id: string) => {
    try {
      ...
      await apiClient(`/business-expenses/${id}`, { method: "DELETE" });
      removeGastoLocal(id);
      return true;
    } catch (err) {
  ...
  const saveChanges = async () => {
    try {
      ...
      for (const gasto of gastosNegocio) {
        ...
        if (gasto.isNew) {
          // Crear nuevo gasto
          await apiClient("/business-expenses", {
            method: "POST",
            body: payload,
          });
        } else {
          // Actualizar gasto existente
          await apiClient(`/business-expenses/${gasto.id}`, {
            method: "PUT",
            body: payload,
          });
        }
      ...
      if (ivaId) {
        await apiClient(`/general-taxes/${ivaId}`, {
          method: "PUT",
          body: {
            nombre: "IVA",
            porcentaje: iva,
