...
import BudgetDetailCard from "@/components/budgets/BudgetDetailCard";
  ...
  const handleConvertToPedido = async () => {
    ...
    try {
      if (budget.estado !== "ACEPTADO") {
        ...
        await budgetService.partialUpdate(budget.id, {
          estado: "ACEPTADO",
        });
      } else {
      ...
      const updatedBudget = await budgetService.getById(budget.id);
