...
export default function ClientDetailPage() {
    ...
    const initialTab = searchParams?.get('tab') === 'history' ? 'history' : 'detail';
    ...
    const loadHistory = async () => {
        try {
            ...
            const clientBudgets = allBudgets.filter(b => b.cliente?.id === clientId);
      ...
      <div className="bg-white px-5 pt-4 pb-2 sticky top-0 z-20">
        <div className="flex items-center gap-4 mb-2">
            ...
            <div className="flex items-center gap-2 text-sm ml-auto">
                ...
                <span className="text-[#C071F4] font-bold text-lg">Detalle cliente</span>
            </div>
        </div>
      </div>
                  ...
                  <div className="flex items-center gap-2">
                    {client.origen?.toLowerCase() === 'telegram' ? (
                      <div className="flex items-center gap-1 bg-blue-50 px-3 py-1.5 rounded-full">
                        ...
                        <span className="text-xs font-bold text-blue-700">Telegram</span>
                      </div>
                    ) : (
                      <div className="flex items-center gap-1 bg-gray-50 px-3 py-1.5 rounded-full">
                        ...
                        <span className="text-xs font-bold text-gray-700">Manual</span>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {activeTab === 'detail' && (
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 animate-in fade-in duration-300">
                    <div className="space-y-1.5">
                        <label className="text-sm text-gray-600 ml-1">Nombre del cliente</label>
                        <input 
                            {...register('nombre')}
                    ...
                    <div className="space-y-1.5">
                        <label className="text-sm text-gray-600 ml-1">Dirección</label>
                        <input 
                            {...register('direccion')}
                            className="w-full bg-white border border-gray-300 rounded-xl px-4 py-3 text-gray-800 outline-none focus:border-[#C071F4] focus:ring-1 focus:ring-[#C071F4] transition-all"
                        />
                  ...
                  {historyLoading ? (
                    ...
                    <div className="space-y-3">
                      {history.map((presupuesto) => (
                        <HistoryAccordion key={presupuesto.id} presupuesto={presupuesto} />
                      ))}
                    </div>
                  )}
                </div>
...
function HistoryAccordion({ presupuesto }: { presupuesto: PresupuestoResponseDto }) {
    const [isOpen, setIsOpen] = useState(false);
    const statusColorMap: Record<string, string> = {
        'BORRADOR': 'bg-gray-400', 'ENVIADO': 'bg-blue-400', 'ACEPTADO': 'bg-green-400', 'RECHAZADO': 'bg-red-400', 'VENCIDO': 'bg-orange-400'
    };
    const formatDate = (d?: string | null) => d ? new Date(d).toLocaleDateString('es-AR') : '-';
    const formatCurrency = (v?: number) => v ? new Intl.NumberFormat('es-AR', { style: 'currency', currency: 'ARS' }).format(v) : '$0';

    return (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            <div onClick={() => setIsOpen(!isOpen)} className="p-4 flex items-center justify-between cursor-pointer hover:bg-gray-50">
                <div className="flex items-center gap-3 flex-1">
                    <span className="font-bold text-gray-900 text-sm">ID: {presupuesto.id}</span>
                    <span className={`${statusColorMap[presupuesto.estado] || 'bg-gray-400'} text-white text-[10px] px-2 py-0.5 rounded-full font-bold`}>{presupuesto.estado}</span>
                </div>
                ...
                <div className="px-5 pb-5 pt-0">
                    ...
                    <div className="space-y-4 text-sm">
                        <div>
                            ...
                            <p className="text-[#C071F4] font-medium">P-{new Date(presupuesto.fechaCreacion).getFullYear()}-{presupuesto.numeroPresupuesto.toString().padStart(4, '0')}</p>
                        </div>
                        {presupuesto.cliente?.nombre && (<div><p className="text-gray-400 text-xs">Cliente</p><p className="text-gray-800 font-medium">{presupuesto.cliente.nombre}</p></div>)}
                        <div className="grid grid-cols-2 gap-4">
                            <div><p className="text-gray-400 text-xs">Total Costo</p><p className="text-gray-800 font-bold">{formatCurrency(presupuesto.totalCosto)}</p></div>
                            <div><p className="text-gray-400 text-xs">Total Venta</p><p className="text-gray-800 font-bold">{formatCurrency(presupuesto.totalVenta)}</p></div>
                        </div>
                        <div><p className="text-gray-400 text-xs">Vencimiento</p><p className="text-gray-800 font-medium">{formatDate(presupuesto.fechaVencimiento)}</p></div>
                        {presupuesto.notas && (<div><p className="text-gray-400 text-xs">Notas</p><p className="text-gray-800">{presupuesto.notas}</p></div>)}
                    </div>
                </div>
            )}
        </div>
