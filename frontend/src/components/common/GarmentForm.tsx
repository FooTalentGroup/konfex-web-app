...
export default function GarmentForm({ collectionId }: GarmentFormProps) {
  ...
  const {
    ...
    isSubmitting,
    submitError,
  } = useAddGarmentForm(collectionId);
  ...
  const onSave = async () => {
    ...
    if (isValid) {
      ...
      const hasDetailErrors =
        errors.image || errors.commercialName || errors.sizes || errors.colors;
      const hasMaterialErrors = errors.rawMaterials;

      if (hasDetailErrors) {
        setActiveTab(0);
        toast.showWarning(
          'Por favor completa todos los campos obligatorios en "Detalle prenda"'
        );
    ...
    <div className="space-y-6 p-4">
      ...
      <div className="p-6 border-t border-gray-200">
        <button
          ...
          className={`w-full bg-[#B65CF2] text-white py-2 px-4 rounded-md hover:bg-[#9a4bc4] focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-[#B65CF2] transition-colors disabled:opacity-50 disabled:cursor-not-allowed ${
