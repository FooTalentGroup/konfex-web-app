...
import { usePlataformaStyles } from '@/hooks/usePlataformaStyles';
...
const ChatHeader: React.FC<ChatHeaderProps> = ({ contact }) => {
  ...
  const plataformaStyles = usePlataformaStyles('telegram');
  ...
  return (
    <div className="w-full px-3 sm:px-4 pt-3 sm:pt-4 pb-3 sm:pb-4 bg-[#FEFCFF] rounded-b-2xl shadow-[0px_3px_5.99px_0px_rgba(0,0,0,0.05)]">
      <div className="flex items-center justify-between mb-2 sm:mb-3 gap-2 sm:gap-3">
        <button
          onClick={() => router.back()}
          className="flex-shrink-0 w-8 h-8 sm:w-9 sm:h-9 md:w-10 md:h-10 rounded-full flex items-center justify-center transition-colors hover:opacity-90 bg-[#9D86AC]"
        >
          <Image
            src="/flechaTela.png"
            alt="Volver"
      ...
      <div className="flex items-center justify-end mb-1.5 sm:mb-2">
        ...
        <div className="relative flex-shrink-0">
          <Image
            ...
            width={32}
            height={32}
            className="rounded-full object-cover flex-shrink-0 w-7 h-7 sm:w-8 sm:h-8 border border-[#8B709D]"
          />
          <div
            className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 sm:w-3.5 sm:h-3.5 rounded-full border-2 border-white ${
              isConnected ? 'bg-green-500' : 'bg-red-500'
            }`}
            title={isConnected ? 'Conectado al backend' : 'Desconectado del backend'}
          />
        </div>
      </div>
