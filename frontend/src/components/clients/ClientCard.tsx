    ...
    `}>
      ...
      `}>
        <div className={`
          ...
          ${isSelected ? 'bg-[#8B709D] border-[#8B709D]' : 'border-[#D5CCDE] bg-white'}
        `}>
      ...
      <div className="flex-1 flex flex-col gap-2 min-w-0">
        ...
        <div className="flex justify-between items-center gap-2 w-full">
          <h3 className={`font-lato font-bold text-[18px] leading-tight transition-colors truncate ${isSelected ? 'text-[#8B709D]' : 'text-gray-900'}`}>
            {name}
          </h3>
          ...
          <button 
            ...
            className={`flex items-center gap-1.5 bg-[#F2D699] text-black px-3 py-1 rounded-full text-[12px] font-lato font-bold shadow-sm transition-all whitespace-nowrap flex-shrink-0
              ${isSelectionMode ? 'opacity-0 pointer-events-none' : 'hover:bg-[#E5C884]'}
            `}
          >
          ...
          {source === 'manual' ? (
             ...
             <button 
                ...
                className={`flex items-center gap-1.5 bg-[#D1EEF3] text-[#2A8FA1] px-3 py-1 rounded-full text-[12px] font-lato font-bold transition-all w-fit
                   ${isSelectionMode ? 'opacity-0 pointer-events-none' : 'hover:bg-[#BEE5EC]'}
                `}
              >
