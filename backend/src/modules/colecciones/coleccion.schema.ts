...
export const createColeccionSchema = z.object({
  body: z.object({
    nombre: z.string().trim().min(1, "El nombre es obligatorio y no puede estar vacío"),

    imagen: z.string().trim().url("La imagen debe ser una URL válida").optional(),

    icono: z.string().trim().url("El icono debe ser una URL válida").optional(),
  }),
});
